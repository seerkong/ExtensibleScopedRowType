...
public class ResumeFiberToken
{
    ...
    public ResumeFiberToken(
        int fiberId,
        IEnumerable<KsNode>? result = null,
        IEnumerable<Instruction>? beforeResumeOps = null)
    {
        FiberId = fiberId;
        Result = result?.ToList() ?? new List<KsNode>();
    ...
    public Fiber? GetNextActiveFiber()
    {
        ...
        if (currentFiber != null)
        {
            ...
            if (currentFiber.IsRootFiber())
            {
                ...
                var rootShouldContinue = topInstruction.OpCode != KsonOpCode.OpStack_LandSuccess;
