...
/// <summary>
/// Represents the state of the Kson interpreter
/// </summary>
public class KsonInterpreterRuntime
{
    /// <summary>
    ...
    /// <summary>
    /// Creates a new Kson state
    /// </summary>
    public KsonInterpreterRuntime()
    {
    ...
    public KsonInterpreterRuntime Copy()
    {
        var result = new KsonInterpreterRuntime
        {
            EnvTree = EnvTree,
    ...
    /// </summary>
    public List<KsNode> OperandStack => FiberMgr.GetCurrentFiber()?.OperandStack.FrameStackView;
    ...
    public KsNode LookupThrowErrIfNotFound(string key)
    {
        var declareEnv = EnvTree.LookupDeclareEnv(GetCurEnv(), key);
    ...
    /// <param name="key">The key to look up</param>
    /// <returns>The value, or null if not found</returns>
    public KsNode LookupFromEnv(Env fromEnv, string key)
    {
    ...
    public void SetVar(string key, KsNode value)
    {
        var declareEnv = EnvTree.LookupDeclareEnv(GetCurEnv(), key);
        declareEnv.Define(key, value);
    }
    ...
    /// <param name="key">The key to set</param>
    /// <param name="value">The value to set</param>
    public void SetVarFromEnv(Env fromEnv, string key, KsNode value)
    {
        var declareEnv = EnvTree.LookupDeclareEnv(fromEnv, key);
        declareEnv.Define(key, value);
    }
