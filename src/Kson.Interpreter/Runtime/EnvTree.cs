...
public class EnvTree : SingleEntryGraph<Env, int>
{
    ...
    /// <summary>
    /// Adds an edge and synchronises the child environment's parent reference.
    /// </summary>
    public override void AddEdge(int prevId, int nextId)
    {
        base.AddEdge(prevId, nextId);

        var parentEnv = GetVertexDetail(prevId);
        var childEnv = GetVertexDetail(nextId);

        if (childEnv != null && !ReferenceEquals(childEnv.ParentEnv, parentEnv))
        {
