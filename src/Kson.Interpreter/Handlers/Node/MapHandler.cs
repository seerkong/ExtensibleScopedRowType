...
{
    public static void ExpandMap(KsonInterpreterRuntime runtime, KsMap map)
    {
        ...
        {
            runtime.AddOp(KsonOpCode.ValStack_PushValue, new KsString(key));
            runtime.AddOp(KsonOpCode.Node_RunNode, map[key]);
        }

        runtime.AddOp(KsonOpCode.Node_MakeMap, map.Count);
        runtime.AddOp(KsonOpCode.ValStack_PopFrameAndPushTopVal, "end make map");
    ...
    public static void RunMakeMap(KsonInterpreterRuntime runtime, Instruction instruction)
    {
        var size = instruction.Memo is int ? (int)instruction.Memo : 0;
        var map = new Kson.Core.Node.KsMap();
        ...
        for (var i = 0; i < size; i++)
        {
            ...
            if (key is Kson.Core.Node.KsString keyString)
            {
