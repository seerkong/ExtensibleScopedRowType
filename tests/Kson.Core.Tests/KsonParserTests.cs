...
public class KsonParserTests
{
  private static void AssertRoundTrip(string source)
  {
    var node = KsonParser.Parse(source);
    var singleLine1 = KsonFormater.SingleLine(node);
    var reparsedSingle = KsonParser.Parse(singleLine1);
    var singleLine2 = KsonFormater.SingleLine(reparsedSingle);
    var prettified = KsonFormater.Prettify(node);
    var reparsedPretty = KsonParser.Parse(prettified);
    var singleLine3 = KsonFormater.SingleLine(reparsedPretty);
  ...
  public void MethodCall()
  {
    ...
    var parsed = KsonParser.Parse(sample);
    var formattedMethod = KsonFormater.SingleLine(parsed);
  ...
  public void PropertyChain()
  {
    ...
    var parsed = KsonParser.Parse(chainSample);
    var formattedMethod = KsonFormater.SingleLine(parsed);
  ...
  public void CallChain()
  {
    ...
    var parsedSampleChain = KsonParser.Parse(sampleChain);
    var formattedChain = KsonFormater.SingleLine(parsedSampleChain);
  ...
  public void ChainUseAsScript()
  {
    const string source = """
        ...
        !(fn |int string|<string int>) // 可以在KsChain前面用手动用! 前缀进行类型标记。也可以用于未来自动分析类型添加类型标记
        (fn #foo :|!int arg1 !string arg2|<string int> // 也可以在声明函数时标记类型
          %[
            // 用 func_name :|arg1 arg2| 表示 函数前缀调用
            // 在执行前缀表达式函数调用时，对KsChain依次求值，先把bar放进操作数栈，然后遍历到KsChain的下个节点，发现是一个前缀函数调用:<> 时，依次求值参数，然后使用操作数栈栈顶的作为函数，后面的作为参数，进行一次函数调用
            (bar :|arg1 arg2|)
