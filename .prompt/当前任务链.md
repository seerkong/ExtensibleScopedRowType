...
目前已经将kson解析器代码放到了src/Kson.Core, 其测试case在tests/Kson.Core.Tests


我希望你能参考我之前编写的一个使用面向数据编程思路，以及参考面向栈的编程语言的双栈（指令栈、操作数栈）思路的解释器，这种解释器因为是面向数据编程，比较容易实现continuation等概念
给kson数据格式，添加脚本执行能力
要参考的解释器，代码在当前项目的kunun.ts下，注意这个解释器的数据格式不是kson，而是有些差别的语法

注意
我只是要用kson格式，参考我原来解释器的解释执行部分的思路，重新写解释器，不能修改kson的解析器。如果有解析问题，请告中断，问我是否要先修复kson解析器的问题再继续
原kunun.ts中的某些逻辑可能有错，以最终能够通过测试case，特别是参考 KsonParserTests.cs 中的 ChainUseAsScript中我期望的目标为导向

请参考kunun.ts执行思路的新的基于kson的解释器，
放到src下, 创建一个Kson.Interpreter project
并在tests下，创建Kson.Interpreter.Tests存放测试case。
在测试case中，应当参考 KsonParserTests.cs 中的 ChainUseAsScript，使用c#的多行文本来编写kson代码

你不可以一次性的把所有kunun.ts的解释思路还原，需要分步
1，先进行数据建模，比如将解释器的XnlState以及关联的下层对象改为KsonLangRuntime
...
我严格要求你，依次先阅读src/Kson.Core 、tests/Kson.Core.Tests 、kunun.ts，再进行代码编写
